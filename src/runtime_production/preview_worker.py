...
def cheap_preview_from_raw(raw_bayer: np.ndarray, target_size: int, quality: str = "low") -> np.ndarray:
    ...
    # Default low quality: 2x2 mean -> grayscale -> resize -> fake BGR
    blocks = raw.reshape(h2 // 2, 2, w2 // 2, 2).astype(np.uint16)
    half_gray = (blocks.sum(axis=(1, 3)) >> 2).astype(np.uint8)  # avg without /4
