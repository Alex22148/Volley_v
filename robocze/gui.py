...
class CaptureGUI(ctk.CTk):
    ...
    def __init__(self, live_q, stats_q, control_q, ready_evt, roles, init_settings,
        ...
        # --- AUTO ROZMIAR I POZYCJA GłĂ“WNEGO OKNA ---
        ...
        # đź”š Koniec inicjalizacji (od teraz eventy z suwakĂłw mogÄ… lecieć do backendu)
        self._initing = False
    ...
    def on_expo_change(self, value, role):
        # đź”’ nie wysył‚aj komend do backendu podczas inicjalizacji widgetĂłw
        if getattr(self, "_initing", False):
            s = self.sliders.get(role, {})
            if s and "exp_val" in s:
                s["exp_val"].configure(text=f"{float(value):.0f} Âµs")
        ...
        try:
            ...
            if s and "exp_entry" in s:
                s["exp_entry"].delete(0, "end"); s["exp_entry"].insert(0, f"{int(float(value))}")
            self._set_status(f" {role}: {float(value):.0f} Âµs", "accent")
        except Exception as e:
            print(f"[GUI][ERROR] on_expo_change({role}) â†’ {e}")
    ...
    def on_gain_change(self, value, role):
        ...
        try:
            ...
            print(f"[GUI][ERROR] on_gain_change({role}) â†’ {e}")

    # --------------------------------------------------------
    # đź–Ąď¸Ź Preview i pętle GUI
    # --------------------------------------------------------
